...
def create_user(user: UserCreate, db: Annotated[Session, Depends(get_db)]):
    result = db.execute(select(models.User.username == user.username))
    ...
    result = db.execute(select(models.User.email == user.email))
...
def post_page(request: Request, post_id: int, db: Annotated[Session, Depends(get_db)]):
    ...
    if post:
        title = post.title[:50]
        return templates.TemplateResponse(request, "blog.html", {"post": post, "title": title})
